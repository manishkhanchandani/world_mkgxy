...
if ((isset($_REQUEST["MM_insert"])) && ($_REQUEST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO push_messages (push_id, from_uid, to_uid, message) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($_REQUEST['push_id'], "int"),
                       GetSQLValueString($_REQUEST['from_uid'], "text"),
                       GetSQLValueString($_REQUEST['to_uid'], "text"),
  ...
  $id = mysql_insert_id();
  $return = array('success' => 1, 'msg' => '', 'message_id' => $id);
